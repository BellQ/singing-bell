...
function gameover() {
    //alert("游戏结束!");
    $("#grid-container").append("<div id='gameover' class='gameover'><p>本次得分</p><span>" + score + "</span><a href='javascript:restartgame();' id='restartgamebutton'>重新开始</a></div>").animate({"opacity":0.2},500);;
    var gameover = $("#gameover");
    gameover.css("width", "500px");
    gameover.css("height", "500px");
    gameover.css("background-color", "rgba(225, 108, 0, 0.5)");
    for (var i = 0; i < 4; i++) {
        var temp1=450-Math.random()*500;
        for (var j = 0; j < 4; j++) {
            var gridCell = $("#grid-cell-" + i + "-" + j);
            var temp=500-Math.random()*1000;
            gridCell.animate({
                "top":temp1,
                "left":temp,
            },500);
    ...
    for (var i = 0; i < 4; i++) {
        var Numemp1=450-Math.random()*500;
        for (var j = 0; j < 4; j++) {
            var numCell = $("#number-cell-" + i + "-" + j);
            var Numemp=500-Math.random()*1000;
            numCell.animate({
                "top":Numemp1,
                "left":Numemp,
                "opacity":0
